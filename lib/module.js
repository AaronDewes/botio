...
function cloneAndMerge() {
  ...
  execWrap('git config --global user.email "beepboop@bot.io"'); // Set up git default email
  execWrap('git config --global user.name "Botio"'); // Set up git default name

  // Merge PR branch into upstream's master
  debug('Running command: git merge PR');
  if (exec('git merge --no-edit PR', {silent:true}).code !== 0) {
